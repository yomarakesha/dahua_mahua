# Elevated helper: add a free secondary IP in 192.168.23.0/24 (camera subnet)
# on the Ethernet adapter. ASCII only (PS 5.1).
$ErrorActionPreference = 'Continue'
$log = 'C:\Users\yomarakesha\Desktop\projects\dss\.netcheck\fix-result-23.txt'
function W($m) { $m | Out-File -FilePath $log -Append -Encoding ascii; Write-Host $m }
('=== camera-subnet IP fix ' + (Get-Date -Format s) + ' ===') | Out-File -FilePath $log -Encoding ascii
...
Get-NetIPAddress -InterfaceIndex $ifx -AddressFamily IPv4 -EA SilentlyContinue |
  Where-Object { $_.IPAddress -like '192.168.23.*' } | ForEach-Object {
...
if ($set) {
    ...
    $p = @(Test-Connection 192.168.23.11 -Count 4 -EA SilentlyContinue)
    W ('ping camera 192.168.23.11 replies: ' + $p.Count + '/4')
} else {
    W 'FAILED: no free IP found in 192.168.23.240-254 / 230-239'
}
W 'DONE'
