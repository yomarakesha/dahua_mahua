          ...
          nvrs.map((n) => {
            ...
            return (
              <button
                ...
                className={[
                  "flex h-10 items-center gap-2.5 rounded-lg border px-2.5 transition",
                  active
                    ? "border-accent/30 bg-accent/[.10]"
                    : "border-white/[.06] bg-white/[.02] hover:bg-white/[.04]",
                ].join(" ")}
              >
                <ChevronRight size={13} className="flex-none text-ink-dim" />
                <span
                  className={[
                    "h-1.5 w-1.5 flex-none rounded-full",
                    healthy
                      ? "bg-accent shadow-[0_0_7px_#2ecc71]"
                      : "bg-ink-faint/60",
                  ].join(" ")}
                />
                <span className="flex-1 truncate text-left text-base font-semibold text-ink-soft">
                  {n.label}
                </span>
